...
/*   Updated: 2024/01/20 22:23:28 by avoronko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_meals(t_args *args)
{
	...
	while (++i < args->data->num_of_philo)
	{
		...
		if (args->data->num_of_meals == -1 || args[i].philo->meals_eaten < args->data->num_of_meals)
		{
...
void	*ft_observer(void *arg)
{
	...
	while (1) 
	{
		...
		while (++i < args->data->num_of_philo) 
		{
			...
			{
				printf("%lu %i %s", (get_current_time() - args->data->start_time), 
					args[i].philo->philo_id, "died\n");
				return (arg);
			}
		}
		if (check_meals(args))
